...
public class DiskTester {
    ...
    /** Testing modes. */
    private enum Mode { READWRITE, READONLY, WRITEONLY };
    ...
    private void usage() {
        debug(0,
            "usage: DiskTester [-qvicVrw] [-n count] id\n"
    ...
    private void run(String[] args) {
        int accessCount = 0;            // -n
        boolean clustered = false;      // -c
        boolean doInit = false;         // -i
        boolean verify = false;         // -V
        long start, stop;
        ...
        // Test library calls getTime, getDiskBlockCount, and getDiskBlockSize
        ...
        // Write a test pattern to all blocks on disk.  Each block contains
        // three integers (in MSB order):  id, blocknumber, zero
        if (doInit) {
        ...
        // Randomly probe n disk blocks (n set by -n option).
        // Choose a block at random and either write  (id, block, serial)
        // or read the block and verify id and block number.
        // If -c is specified, 90% of probes are to 10% of the blocks.
        // Otherwise, access is uniform.
        if (accessCount > 0) {
            ...
            for (int i = 0; i < accessCount; i++) {
                ...
                if (clustered) {
                    if (rand.nextDouble() < 0.9) {
                        ...
                        b = (hotBase + hotSize + rand.nextInt(dsize - hotSize))
                            % dsize;
                    }
                } else {
    ...
    private static void check(int i) {
        if (i != 0) throw new RuntimeException("call failed");
    } // check(int)
